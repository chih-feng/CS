...
function OnRcvVdMsg($aryPara)
{
	...
	switch(	$head ){
		...
		case "TST" ://2020.5.62335
			...
			$Ap_Vd->VDSend("WSS1", "-LTA" . $stat);
			$Ap_Vd->VDSend("IOB1", "-LTA" . $stat);
			break;
		case "?" :
		default:
			echo	"-----\n"
				.	"DI=A+ : 'DI=' 'A' '+'\n"
				.	"PTM1  : set PTW_Mode, 0:Test IO,1:Put same sku in once,2:one by one\n"
				.	"-----\n"
				;
	}
...
function Do_GetDI($di_name , $stat) // 'A'  , '+'
{
	...
	# do location put
	if($do_name){
		PTW_Checkin($do_name);//'A3'
	}


}
function PTW_Checkin($loc) // DO_name == Loc_id
{
	global $Ap , $Ap_Vd  ;
	...
	if($Ap->PTW_Mode==PTW_MODE_IO) {
		// turn the lite Off
		$Ap_Vd->VDSend("WSS1" ,"-LTE-$loc" );// "LTE-A3"
		SendDO2IoB($loc , '-');
		return;
	}
	//if($loc == $Ap->Loc_I)
...
function SendDO2IoB($do_name , $stat)
{
	global $Ap , $Ap_Vd  ;

	$vd_iob		= $Ap->DO_Map[$do_name]["IOBX"] ;
	if(!$vd_iob) {
