...
function Tmr_Blink_DO()//function setting in VD_xxxx.cfg "Interval"
{
	...
	*/
	...
	foreach( $Ap->Blink_List as $do_name => $stat){//## [ 'A2' => '+' ]
		$Ap->Blink_List[$do_name] = $stat == "-" ? "+" : "-" ;
		$Vd_Mbs->Set_DO(  $do_name  , $Ap->Blink_List[$do_name] );
	}
...
function OnRcvVdMsg($aryPara)
{
	...
	switch ($cmd) {//
		...
		case 'BLK': //Set DO Blink, ex."BLKA5+" set DO A5 blink
			$stat 		= substr($Mesg, 5 , 1 );
			$isEnabled 	= $stat  == '+' ? 1 : 0 ; 
			$Do_name 	= substr($Mesg, 3,2); //GET Blink DO name, ex. "A5" "AR" AR for Alart92 DO)
			$Ap->Blink_List[$Do_name] = $stat ; //[ "A5" => '-' ]
			$Idx 	= $Vd_Timer->FindIntvlTableIdx("Blink_DO");
			if ($Idx != -1) {
				$Vd_Timer->IntvlTable[$Idx]["Enable"] = $isEnabled ; 
			}
			if(!$isEnabled){
				unset($Ap->Blink_List[$Do_name]);
