...
function ParseRcvBuffer()
{
	...
	for($i = 0 ; $i < $aryCount ; $i++){
		...
		$Ap_Vd->VDSend("WSS1" , "-BCR=". $wgt_data);//+1234.56
...
function OnRcvVdMsg($aryPara)
{
	...
	$cmd 		= substr($Mesg , 0, 4);
	switch ($cmd) {
		case 'WRTS'://write string to tcp, for test WGT1
			$data 	= substr($Mesg ,  4);
			$ret 	= fwrite($Ap_TCP_cts->Cts_Client , $data);//寫入RS232//寫入RS232
		...
		case 'WRTN'://write string + \r\n to nport, for test WGT1
			$data 	= substr($Mesg ,  4). "\r\n";
			$ret 	= fwrite($Ap_TCP_cts->Cts_Client , $data);//寫入RS232
			//echo  __FILE__ ." # ". __LINE__ . ".\n\$Ret=$ret.[$data]\n";
